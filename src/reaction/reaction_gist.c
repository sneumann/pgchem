...
{
   return 1.0f - ob_tanimoto((uint8 *) fp1->fp, (uint8 *) fp2->fp, 2*FPSIZE2*sizeof(uint32));
}
  ...
  if (entry->leafkey)
    {
      ...
      memcpy (fp->fp, rxn->fp, 2*FPSIZE2*sizeof(uint32));
